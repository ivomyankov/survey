    ...
    @foreach($element as $key => $option)
        ...
        @else    
            <div class="form-check px-4 my-2" style="background-color: #f8f9fa;" alt="{{$option->id}} ">
                @if($element[0]->type == 'checkbox')
                    <input class="form-check-input q{{$element[0]->id}}" type="{{$element[0]->type}}" value="{{$option->id}}" name="q{{$element[0]->id}}[]" id="q{{$element[0]->id}}_{{$option->id}}">                        
                @else
                    <input class="form-check-input q{{$element[0]->id}}" type="{{$element[0]->type}}" value="{{$option->id}}" name="q{{$element[0]->id}}" id="{{$option->id}}">
